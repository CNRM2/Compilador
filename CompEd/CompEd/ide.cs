    ...
    public partial class Ide : Form
    {
        ...
        public void AnlzdrSntctc(string[] sentencias)
        {
           ...
           for (int i = 1; i < sentencias.Length; i++)
           {
               ...
               {
                   if (Regex.IsMatch(sentencias[i], @"^int|integer\s+[a-z](\s+:\s+\d(0,32000))*;$"))
                   {
                   ...
                   else if (Regex.IsMatch(sentencias[i], @"^double|real\s+[a-z](\s+:\s+\d(0,32000))*;$"))
                   {
                   ...
                   else if (Regex.IsMatch(sentencias[i], @"^string|texto\s+[a-z](\s+:\s+[a-z](1,15)')*;$"))
                   {
                       System.Windows.Forms.MessageBox.Show("esto es una sentencia string");

                   }
                   else if (Regex.IsMatch(sentencias[i], @"^bool|boolean\s+[a-z](\s+:\s+(true|false))*;$"))
                   {
                   ...
                   else if (Regex.IsMatch(sentencias[i], @"if\s\(\s+\w+\s(<|>|<:|>:|::|!:)\s\w+\s\)\s\{$"))//--
                   {
                       MessageBox.Show("comienzo de if");
                   }
                   else if (Regex.IsMatch(sentencias[i], @"elseif\s\(\s+\w+\s(<|>|<:|>:|::|!:)\s\w+\s\)\s\{$"))//--
                   {
                       MessageBox.Show("comienzo de else if");
                   }
                   else if (Regex.IsMatch(sentencias[i], @"else\s*\{$"))//--
                   {
                       MessageBox.Show("comienzo de else");
                   }
                   else if (Regex.IsMatch(sentencias[i], @"switch\s\(\s\w+\s(<|>|<:|>:|::|!:)\s\w+\s\)\s\{$"))//--
                   {
                       MessageBox.Show("comienzo del switch");
                   }
                   else if (Regex.IsMatch(sentencias[i], @"case\s\(\s(\w+|\d+)\s\)\s{$"))//--
                   {
                       MessageBox.Show("comienzo de case");
                   }
                   else if (Regex.IsMatch(sentencias[i], @"case\s;$"))//--
                   {
                       MessageBox.Show("break del case");
                   }
                   else if (Regex.IsMatch(sentencias[i], @"while\s\(\s\w+\s(<|>|<:|>:|::|!:)\s\w+\s\)\s{$"))
                   {
                       MessageBox.Show("inicio de un while");
                   }

                   else if (Regex.IsMatch(sentencias[i], @"printf\s\(\s(\w*)|'\w*'\)\s;$"))
                   {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (PagCodigo.Text.StartsWith("Main()"))
                {
                    guardaArchivo2(nomarchivox);
                    tabla_simbolos.reinicialista();
                    tabla_errorres.reinicialista();
